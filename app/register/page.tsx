...
export default function RegisterPage() {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        setMessageType("error");
        setMessage(
          data.message ?? "לא ניתן לשלוח את בקשת ההצטרפות"
        );
        return;
      }
  ...
  if (isSubmitted) {
    return (
      ...
      >
        <div className="absolute right-[-120px] top-[-120px] h-[340px] w-[340px] rounded-full bg-purple-600/20 blur-[100px]" />

        <div className="absolute bottom-[-140px] left-[-100px] h-[360px] w-[360px] rounded-full bg-amber-400/10 blur-[110px]" />
      ...
      <section className="relative z-10 w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-white/55">
            מלא את הפרטים ובחר קוד אישי. הכניסה תתאפשר
            לאחר אישור מנהל הקבוצה.
          </p>
        </div>
